        ...
        for line in atemp:
                ...
                if cond == 0:
                        if procurarletra(l,'M') == 82:
                                cond4 = 1
                        if procurarletra(l,'G') == 92 and cond4 == 1:
                                cond5 = 1
                                newfile.write('\n' + mudarvalorlinha(l,'E',ler('tmp/E.dat')))
                        if procurarletra(l,'G') != 28 and procurarletras(l,'G','Z') == 0 and procurarletra(l,'G') != 32 and cond5 == 0: #delete Z changes 
                                newfile.write('\n' + l)
                        if procurarletra(l,'F') >= 500 and procurarletras(l,'G','Z') == 0:
                                cond = 1
                        cond5=0
                if cond2 == 0:
                ...
                if cond3 == 0:
                        if cond2 == 1 and (l+'\n') == xy: #find XY (after find Z)
                                cond3 = 1
                if cond3 == 1:
                        if procurarletra(l,'Z') > 0:
                                newfile.write('\n' + mudarvalorlinha(l,'Z',str(round(procurarletra(l,'Z')-float(ler('tmp/z.dat')),6))))
                        else:
...
def imprimir(line,impressora):
        ...
        if procurarletras(l,'X','Y') == 1: #save X, Y, E e define used mass
                if procurarletra(l,'E') > 0:
                        ...
                        massa = comprimento * math.pi * ((0.175/2)**2) * float(ler('tmp/densidade.dat'))
                        try:
                                Eabs=ler('tmp/Eabs.dat')
                                gravar(str(procurarletra(l,'E')),'tmp/E.dat')
                                massat = massa
                        except IOError:
                                massat = float(ler('tmp/massa.dat')) + massa
...
while True:
        ...
        while true==0: #try copy file from 'arquivos' to be printed
                try: #verify if some file being printed 
                        ...
                        try:
                                ...
                                d=0
        s = serial.Serial('COM5"',115200) #open serial port
        print ("Iniciando...")
        serialcmd="\r\n\r\n" #turn printer on
        ...
        try: #resume print
                ...
                f = open('tmp/temporario.gcode','r')
                linhaAtual = int(ler('tmp/linhaAtual.dat'))-1
                for line in f:
